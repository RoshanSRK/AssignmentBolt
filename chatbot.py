...
def get_bot_response(user_input):
    ...
    if "hello" in user_input:
        ...
        return "I'm just Bob, but I'm doing great! How about you?"
    elif "bye" in user_input:
        return "Goodbye! Have a nice day!"
    # elif "display database" in user_input:
    #     return "Fetching database records..."
    elif "start bobcheck" in user_input:
        return "Starting the bobcheck on candidates..."
    else:
...
def remind_customers(window):
    try:
        ...
        uipath_workflow_path = r"D:\rsrko\Documents\UiPath\Assignment Bolt\Main.xaml"
...
def handle_bob_check():
    # Display the bot response before triggering the reminder
    chat_area.config(state=tk.NORMAL)
    chat_area.insert(tk.END, "Bot: Starting bobcheck...\n\n")
...
def create_chatbot_gui():
    ...
    def send_message(user_input=None):
        ...
        if "start bobcheck" in user_input.lower():
            handle_bob_check()
    ...
    remind_customers_button = tk.Button(window, text="Start bobcheck", command=lambda: handle_bob_check())
